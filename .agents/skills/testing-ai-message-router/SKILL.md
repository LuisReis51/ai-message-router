...
### 9. UI Visibility / Contrast Testing

When testing visual changes to the 3D frontend (CSS colors, Three.js material properties):

**Key elements to verify:**
- **3D particles**: Should be clearly visible as distinct colored dots/squares against the #0a0a1a background. If particles appear as invisible dust, check `PointsMaterial.size` (should be >= 0.10) and `opacity` (should be >= 0.8).
- **Wireframe sphere**: Faintly visible in center. `MeshBasicMaterial.opacity` should be >= 0.15.
- **Orbital rings**: Thin colored arcs. `MeshBasicMaterial.opacity` should be >= 0.20.
- **Text contrast**: All text on dark background must be >= #999 for body text, >= #b8b8b8 for subtitles, >= #c0c0c0 for headings. Colors like #444/#555 are unreadable on #0a0a1a.
- **Placeholder text**: Should be >= #777 to be visible in dark-themed textareas.

**Testing approach:**
1. Use browser GUI with `zoom` action to inspect text readability at close range
2. Verify particles are individually distinguishable (not a faint haze)
3. Check provider chips remain visible with colored dots and readable labels (regression)
4. Compare before/after if possible — the user reported "foggy letters" and "intro disappearing" with old values

**Threshold reference (dark background #0a0a1a):**
- Unreadable: #444, #555 (contrast ratio < 3:1)
- Barely readable: #666, #777 (acceptable for placeholders only)
- Clearly readable: #999, #aaa, #b8b8b8, #c0c0c0 (good for body/headings)
...
- **UI visibility on dark theme**: Text colors below #777 may be invisible on the #0a0a1a background. If user reports "foggy" or "disappearing" text, check CSS color values in `app/static/index.html` styles section.
